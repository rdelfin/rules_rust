    ...
    fn with_overrides(mut self, extras: &BTreeMap<CrateId, PairredExtras>) -> Self {
        ...
        if let Some(pairred_override) = extras.get(&id) {
            ...
            if let Some(skipped_deps) = &crate_extra.skipped_deps {
                println!("ID: {id}");
                println!("\tDeps: {:?}", self.common_attrs.deps);
                println!("\tSkipped deps: {skipped_deps:?}");
                for skipped_dep in skipped_deps {
                    let skipped_dep_copy = skipped_dep.clone();
                    let dep_filter = |dep: &CrateDependency| dep.id.name == skipped_dep_copy;
                ...
                println!("\tDeps after: {:?}", self.common_attrs.deps);
            }
